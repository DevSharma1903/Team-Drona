...
#include <Watchdog_t4.h>  // NEW: Hardware watchdog
...
// NEW: UART communication timeout
const unsigned long UART_RX_TIMEOUT_MS = 200;  // 200ms without UART = emergency

// NEW: Servo stuck detection
const unsigned long SERVO_STUCK_THRESHOLD = 100;  // 100 cycles = 1 second at 100Hz
...
void watchdogTimeout() {
  // CRITICAL: System hung - force servos to safe position before reset
...
void startupCenteringSequence() {
  ...
  for (int i = 0; i <= STEPS; i++) {
    wdt.feed();  // Pet watchdog during startup
...
// ============================================================================
// SAFETY WATCHDOG - ENHANCED
// ============================================================================
void checkSafetyWatchdog() {
  unsigned long now = millis();
  
  // Check 1: Touch watchdog
  ...
  // NEW: Check 2: UART communication timeout
...
// ============================================================================
// PI(D) CONTROL - ENHANCED with logging
// ============================================================================
void computeControl() {
  if (emergencyStop) {
    return;
  ...
  // NEW: Log saturation for debugging
  ...
  plateAngleX_cmd = KP_X * errorX_mm + 
                    KI_X * integralX - 
                    KD_X * ballVX_mms;
  
  plateAngleY_cmd = KP_Y * errorY_mm + 
                    KI_Y * integralY - 
                    KD_Y * ballVY_mms;
...
// ============================================================================
// SERVO UPDATE - ENHANCED with stuck detection
// ============================================================================
void updateServosWithSlewLimit() {
  ...
  int servoX_us = SERVO_CENTER + 
                  (int)round((servoAngleX_actual + SERVO_X_CENTER_OFFSET) * SERVO_US_PER_DEGREE);
  int servoY_us = SERVO_CENTER + 
                  (int)round((servoAngleY_actual + SERVO_Y_CENTER_OFFSET) * SERVO_US_PER_DEGREE);
  
  servoX_us = constrain(servoX_us, SERVO_MIN, SERVO_MAX);
  servoY_us = constrain(servoY_us, SERVO_MIN, SERVO_MAX);
  
  // NEW: Detect stuck servo
  if (abs(servoX_cmd - servoAngleX_actual) > 1.0f) {
    if (abs(servoX_us - lastServoX_us) < 1) {
      ...
      if (servoStuckCount > SERVO_STUCK_THRESHOLD) {
        ...
        servoStuckCount = 0;  // Reset to avoid spam
      }
    } else {
      servoStuckCount = 0;
    }
...
void sendUartData() {
  ...
  // NEW: Update last RX time when UART is active
...
void printDebug() {
  ...
  if (ballDetected) {
    ...
    float error = sqrt(pow(TARGET_X_MM - ballX_mm, 2) + 
                      pow(TARGET_Y_MM - ballY_mm, 2));
...
void setup() {
  ...
  // NEW: Initialize hardware watchdog
  wdtConfig.window = 0;
  wdtConfig.timeout = 500;  // 500ms timeout
